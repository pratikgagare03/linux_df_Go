...
func TestGetFormattedDFOutput(t *testing.T){
	t.Run("TestEmptyInput", func(t *testing.T) {
		var input []Fs

		_, err := GetFormattedDFOutput(input, 0)
	...
	t.Run("WrongIndex", func(t *testing.T) {
		...
		output, err := GetFormattedDFOutput(input1, input2)

		require.Nil(t, err, "Unexpected error for handled invalid input2")
